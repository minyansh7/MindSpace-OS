# Publish drafts — mindfulness-space UX session (2026-04-21)

Live app: https://mindfulness-space-x.streamlit.app {{CONFIRM: URL}}
Repo: {{CONFIRM: whether to share the GitHub repo publicly, and its URL}}
CLAUDE.md: {{CONFIRM: GitHub path once repo is public, e.g. github.com/minyansh/mindfulness-space/blob/main/CLAUDE.md}}
...
### Title candidates
A) **Pathways, Trees, Web, Currents: Renaming a Data Site Until It Read Like One Voice**
B) **A Quiet UX Pass on MindSpace OS: Fewer Knobs, Sharper Names, Mobile That Respects Desktop**

### Post body (pick title, then use below)

# Pathways, Trees, Web, Currents
...
> image: [sidebar before / after — both screenshots stacked, left = "Cocurrence Mapping*" names, right = Pathways/Trees/Web/Currents]
...
- Dropped the Plotly modebar across the narrative charts (`displayModeBar: False`). It was chartjunk on a scrolltelling page.
...
I also replaced a fragile 150-line hand-rolled "button-dot slider" (held together with `!important` CSS) with a single native `st.select_slider`. The filters on Narrative Trees that used to be decorative HTML `<div>`s are now a real `st.slider` and `st.radio` that actually filter `df_edges`. A reader who moves a knob should see the chart move.

> image: [Theme Web before / after — left shows ~100-node hairball, right shows clean 40-node default with toggle visible]
...
Design-intent notes (CLAUDE.md): {{CONFIRM: GitHub link}}
...
1/7 Spent 13 hours this weekend on a quiet UX pass across MindSpace OS — a Streamlit data-storytelling site on r/meditation discussions. 26 commits. No redesign. Mostly: naming, typography, and a mobile path that never touches desktop. Notes below.
*(screenshot cue: hero shot of the live site homepage)*

2/7 Before: Cocurrence Mapping. Cocurrence Mapping Over Time. Cocurrence Mapping complex. Main Topics Sankey.
...
*(screenshot cue: sidebar before/after, stacked)*

3/7 Removed a subtitle I liked: "Living Narrative Intelligence Platform."
It was competing with the H1 for meaning. Replaced with a functional eyebrow — `QUARTER 2025Q2` — matching the TIME TRAVEL eyebrow above the slider. Subtitles earn their line or they go.

4/7 Killed a 150-line hand-rolled "button-dot slider" held together by `!important` CSS. Replaced with one native `st.select_slider`. Also turned Narrative Trees filters from decorative HTML divs into a real `st.slider` + `st.radio` that actually filter `df_edges`.

5/7 Mobile without breaking desktop: `mobile.py` reads User-Agent. Plotly pages branch render calls. The four `components.html` iframes get `@media (max-width: 768px)` CSS injected inline. Desktop never matches the query, so rendering is bit-identical.
*(screenshot cue: side-by-side desktop/mobile on Theme Currents)*

6/7 On descriptive H1s: Borkin et al. argue for declarative titles (people recall them). But apply that to a six-page site and the sidebar stops being an anchor. Kept brand H1s; let the subtitle carry the finding. H1 stability is a site-level concern, not a chart concern.

7/7 Full writeup — what got renamed, what got cut, the mobile rollout, and why Theme Web now caps at 40 nodes by default: {{CONFIRM: Substack link}}
...
(11 words. Works as Substack subhead and Twitter bio line.)

---

## 4. Five punchy highlights

- 26 commits. Zero desktop pixels changed on mobile.
- Renamed 4 pages into one metaphor family.
- Killed 150 lines of `!important` CSS slider hack.
- Capped Theme Web at 40 nodes; first paint stays calm.
- Stripped ~13 decorative emojis from one Sankey alone.

---

## 5. Screenshot shot-list

| Image | Page / view | Suggested size | Where to use |
|---|---|---|---|
| Sidebar before/after | Sidebar (Cocurrence* names vs. Pathways/Trees/Web/Currents) | 800×1200, stacked | Substack §"A family of names"; Tweet 2/7 |
| Theme Web default vs. show-all | Theme Web page, 40-node default and `Show all` toggle expanded | 1600×900, side by side | Substack §"Static decluttering"; Tweet optional after 5/7 |
| Narrative Trees title stack | Narrative Trees top-of-page with H1 + `QUARTER 2025Q2` eyebrow + TIME TRAVEL slider | 1600×600 | Substack §"One H1, one subtitle" |
| Theme Currents desktop vs. mobile | Theme Currents iframe rendered on desktop (1440px) next to mobile (390px) | 1800×1000 composite | Substack §"Mobile, without touching desktop"; Tweet 5/7 |
| Homepage hero | Homepage top fold on live site | 1600×900 | Tweet 1/7 pinnable |
