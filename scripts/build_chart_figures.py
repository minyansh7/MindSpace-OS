...
<body>
    ...
    function applyArchetypeLabelMode() {{
        ...
        const narrow = w <= 1024;
        // Offset scale: 0 at 320 (label sits ON cluster), 1.0 at 1100+ (full
        // desktop offset). Linear interpolation keeps labels within the
        // visible chart bounds at small viewports where text-center
        // anchoring + outward offsets used to push labels off-screen.
        const offsetScale = Math.max(0, Math.min(1.0, (w - 320) / 800));
